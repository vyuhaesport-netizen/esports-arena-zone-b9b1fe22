-- Drop and recreate the function to accept prize distribution as parameter
...
BEGIN
  ...
  -- Use provided prize distribution or fall back to tournament's stored distribution
  v_prize_dist := COALESCE(p_prize_distribution, v_tournament.prize_distribution);
  
  -- Update the tournament's prize distribution if a new one was provided
  IF p_prize_distribution IS NOT NULL THEN
    -- Validate total distribution doesn't exceed prize pool before updating
    SELECT COALESCE(SUM((value)::numeric), 0) INTO v_total_distributed
    FROM jsonb_each_text(p_prize_distribution);
    ...
    SET prize_distribution = p_prize_distribution, updated_at = now()
    WHERE id = p_tournament_id;
  END IF;
  
  -- Reset for actual winner calculation
  ...
  LOOP
    ...
    v_prize_amount := COALESCE((v_prize_dist->>v_position::text)::numeric, 0);
