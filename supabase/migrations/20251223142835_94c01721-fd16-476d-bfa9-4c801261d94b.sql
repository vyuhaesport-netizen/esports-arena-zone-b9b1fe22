-- Update process_winner_declaration to use Dhana system instead of wallet
...
BEGIN
  ...
  LOOP
    ...
    IF v_prize_amount > 0 THEN
      ...
      -- Update winner's wallet atomically (prize money goes to player wallet)
      UPDATE profiles
      SET 
  ...
  -- Handle organizer earnings - NOW CREDITS TO DHANA INSTEAD OF WALLET
  v_organizer_earnings := COALESCE(v_tournament.organizer_earnings, 0);
  
  IF v_organizer_earnings > 0 THEN
    -- Credit Dhana to organizer (15-day settlement period)
    SELECT credit_dhana_commission(
      p_organizer_id,
      p_tournament_id,
      v_organizer_earnings,
      'Commission for tournament: ' || v_tournament.title
    ) INTO v_dhana_result;

    -- Notify organizer about Dhana credit
    INSERT INTO notifications (
      user_id,
      type,
      title,
    ...
    VALUES (
      ...
      'dhana_earned',
      '💰 Dhana Earned!',
      'You earned ' || v_organizer_earnings || ' Dhana from "' || v_tournament.title || '". It will be withdrawable after 15 days.',
      p_tournament_id
    );
  END IF;
  ...
  RETURN json_build_object(
    ...
    'organizer_dhana', v_organizer_earnings,
    'message', 'Winners declared successfully! Total ₹' || v_total_distributed || ' distributed. Organizer earned ' || v_organizer_earnings || ' Dhana.'
  );
...
-- Also update process_team_winner_declaration to use Dhana
CREATE OR REPLACE FUNCTION public.process_team_winner_declaration(p_tournament_id uuid, p_organizer_id uuid, p_team_positions jsonb)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF v_tournament.status = 'upcoming' THEN
    RETURN json_build_object('success', false, 'error', 'Cannot declare winners for upcoming tournament');
  END IF;
  ...
  LOOP
    ...
    IF v_team_members IS NULL OR array_length(v_team_members, 1) IS NULL THEN
      CONTINUE;
    END IF;
    ...
    LOOP
      ...
      VALUES (v_member_id, 'prize', v_prize_per_member, 'completed',
              'Team ' || v_team_name || ' Position #' || v_position || ' prize for ' || v_tournament.title);

      INSERT INTO notifications (user_id, type, title, message, related_id)
      VALUES (v_member_id, 'prize_won', '🏆 Team Victory!',
              'Your team "' || v_team_name || '" won Position #' || v_position || '! ₹' || v_prize_per_member || ' credited.',
              p_tournament_id);
  ...
  -- Handle organizer earnings - NOW CREDITS TO DHANA
  v_organizer_earnings := COALESCE(v_tournament.organizer_earnings, 0);
  ...
  IF v_organizer_earnings > 0 THEN
    ...
    INSERT INTO notifications (user_id, type, title, message, related_id)
    VALUES (p_organizer_id, 'dhana_earned', '💰 Dhana Earned!',
            'You earned ' || v_organizer_earnings || ' Dhana from "' || v_tournament.title || '". Withdrawable after 15 days.',
            p_tournament_id);
  END IF;
